...
>
> ⚠️ **이행 중(2026-08-15)** — `api.1pass.dev` 는 아직 `com.dcodelabs.logi` 로
> **verified** 상태다(기기에서 `pm get-app-links` 로 확인). claim 제거는 **모든 RP 가
> 호스트 분리 SDK 로 올라간 뒤**다 — 먼저 걷어내면 구버전 SDK 를 쓰는 RP 의 앱 자동
> 점프가 즉시 끊긴다. 그동안 폴백은 SDK 가 Custom Tab 을 **브라우저 패키지에 고정**해
> 지킨다(고정하지 않으면 verified 앱이 ACTION_VIEW 를 브라우저보다 먼저 가져간다).
>
...
- `v1.3.x` — current stable. minSdk 23, target SDK 35, Kotlin 2.1+, JDK 17.
